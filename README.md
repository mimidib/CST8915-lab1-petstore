...
**Purpose: What is this service responsible for?**
    Order service is a web app responsible for listening and receiving client orders from store-front and sending them to a Queue (RabbitMQ) to be processed by any subscribers, while notifying clients of successful order placement.
**Technology Stack: Which language/framework does it use and why?**
- RabbitMQ using AMQP (Advanced Message Queuing Protocol): A client library to communicate with RabbitMQ. RabbitMQ is a message broker/message queueing software that allows different microservices to communicate asynchronously. It receives data or tasks from producers, stores them in queues, and delivers them to consumers to provide higher performance, load balancing, and reliability.
- CORS (Cross-Origin Resource Sharing) is a browser security feature that APIs must implement to allow cross-origin requests. Browsers reject cross-origin requests by default (same-origin policy) to prevent malicious websites from making unauthorized API requests using your logged-in session and potentially accessing your data on other sites.
- Express.js is a simple framework to create web apps in JavaScript (like Flask)
...
RabbitMQ helps create a microservice architecture by decoupling services from each other so they don't need to know about each other to function and communicate, allowing them to move on without waiting for a process to complete.

**Inter-Service Communication: How does it interact with other services?**
- RabbitMQ's How: In the API that sends a message (we have no idea which subscriber will receive it), we import the library, set the URL, and connect to the RabbitMQ server on the VM using localhost. To send a message to the queue, we declare the channel for communication and declare the queue. We convert the message (the order object) to a JSON string and create the queue (ensuring it doesn't persist after restart), then send the message into the queue with logging and send a response to the client confirming order receipt. This allows any subscribers to be notified of the order without order-service waiting for processing or even knowing subscribers exist, further confirming our microservices decoupling.

- CORS' How: CORS works by using two types of requests: simple requests and non-simple requests (preflight). Depending on the request type and content type, APIs must add CORS headers for browsers to access them. If an API allows browser access using Express with CORS middleware, it must allow specific origins by specifying either `app.use(cors())` to accept all origins (specifically for development), or for production, specify:
  1. The `origin` - allows origins
  2. `methods` - permitted specific actions
  3. `credentials: true` - if using cookies/auth

For non-simple requests, headers must be set more explicitly. Using Express and CORS middleware allows for preflight CORS processing before each request and sends the CORS headers in each response rather than requiring an OPTIONS route handler for every endpoint.  
...
**Architecture Role: How does it fit into the microservices architecture?**
Product-service is a web app that serves the simple purpose of providing a list of available products.
It serves a list of static products to anyone who requests it, without being tightly coupled to other services within the pet-store repository. It decouples product data from the rest of the system, allowing other parts to stay independent and be deployed separately since its runtime (Tokio) is separate and uses an entirely different language from the rest of the repository.   
...
**Technology Stack: Which language/framework does it use and why?**
- Vue.js: A JavaScript framework used for its reactive data-binding and component-based architecture. It has features like computed properties, which we use for `totalPrice` to calculate and return product price × quantity if truthy (if it's not null, false, 0, undefined, etc.), else returns 0 without refreshing the page
- Fetch API: Used to handle asynchronous communication with backend services
**Architecture Role: How does it fit into the microservices architecture?**
...
- babel.config.js is a JavaScript setting for compatibility
- jsconfig.json is another JavaScript file, this time for configurations
- package-lock.json is for dependency versions

**Inter-Service Communication: How does it interact with other services?**
Store-Front talks to both Product-Service & Order-Service:
- An async `fetchProducts()` method uses `fetch('http://20.104.102.185:3030/products')` to perform a GET to the `/products` path of the product-service on the VM's public IP and port (allowed because of CORS)
- Another async `submitOrder()` method uses `fetch('http://20.104.102.185:3000/orders')` to perform a POST to the `/orders` path of order-service (allowed because of CORS) on the VM's public IP and port
